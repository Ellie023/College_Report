...
void huffman_traversal(TreeNode *node,int codes[],int top) {

    if(node->l){
        codes[top]=0;
        huffman_traversal(node->l,codes,top+1);
    }
    if(node->r){
        codes[top]=1;
        huffman_traversal(node->r,codes,top+1);
    }
    if(is_leaf(node)){
...
void huffman_encoding(char *str, bits_stream *bits_str)
{
    ...
    while(str[j]!='\0'){
        ...
        k=0;
        for(int i=0; i<m_bit_size[n]; i++){
            bits_str->stream[a++]= (m_LUT[n][k++]);
            interval++;
        }
...
{
    int index_char=0,i=0;
    TreeNode *init=node;

    while(i<bits_str->length){
...
int main()
{
    ...
    int *codes = (int *) malloc(sizeof(int)*MAX_BIT);
    ...
    char *decoded_str=(char *) malloc(sizeof(char )*MAX_CHAR);
    ...
    //start decoding
    int decoded_char_length=decoded_char_length = huffman_decoding(bits_str1,  element_root.ptree, decoded_str);
    free(m_bit_size);
    free(m_LUT);
    free(codes);
    free(bits_str1);
